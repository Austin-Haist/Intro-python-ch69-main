from catalog import store_items # import dictionary from catalog
...
def print_catalog():
    header("- Our Catalog -")
    for prod in store_items:  # Ljust means left justif, so 15 spaces to the right
...
def add_product_to_cart(prod_id):
    ...
    for prod in store_items:
        if str(prod["id"]) == str(prod_id): # matches user option to dictionary product
            ...
            break # stops after finding the product

    if not found:
        print("**ERROR: product dosent exist")
